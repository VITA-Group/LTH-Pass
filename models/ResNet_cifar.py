    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                """
                self.shortcut = LambdaLayer(lambda x:
                                            F.pad(x[:, :, ::2, ::2], (0, 0, 0, 0, planes//6, planes//6), "constant", 0))
            elif option == 'B':
                self.shortcut = nn.Sequential(
                     nn.Conv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False),
                     nn.BatchNorm2d(self.expansion * planes)
...
class ResNet_small(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(3, 13, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(13)
        self.layer1 = self._make_layer(block, 13, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 17, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 25, num_blocks[2], stride=2)
...
def resnet20_small(number_class=10):
    print('* remain weight = 21.08 %')
    return ResNet_small(BasicBlock_small, [3, 3, 3], num_classes=number_class)
